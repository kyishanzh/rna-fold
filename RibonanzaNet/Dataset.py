...
def load_bpp(filename,seq_length=177):
    """
    Load base pair probabilities from a file and create a matrix.
    
    Args:
        filename (str): Path to the BPP file
        seq_length (int): Length of the RNA sequence
        
    Returns:
        np.ndarray: Matrix of base pair probabilities
    """
...
class RNADataset(Dataset):
    """
    Dataset class for RNA sequences and their reactivity data.
    
    Args:
        indices (list): List of indices to use from the data dictionary
        data_dict (dict): Dictionary containing sequence data and labels
        k (int): Window size for attention mask
        train (bool): Whether this is training data
        flip (bool): Whether to allow sequence flipping for augmentation
    """
    ...
    def generate_src_mask(self,L1,L2,k):
        """Generate source mask for attention mechanism"""
...
class TestRNAdataset(RNADataset):
    """
    Dataset class for RNA test data, inheriting from RNADataset.
    Contains simplified __getitem__ method for test-time inference.
    """
    def __getitem__(self, idx):
...
class Custom_Collate_Obj:
    """
    Collate function for training data batches.
    Handles padding and stacking of various features.
    """
...
class Custom_Collate_Obj_test(Custom_Collate_Obj):
    ...
    Simplified version of Custom_Collate_Obj for inference.
    """
    def __call__(self,data):
        length=[]
        for i in range(len(data)):
